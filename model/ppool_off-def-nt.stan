// Model with separate offense and defense player effects and no team effects.
...
transformed data{
  //Obtain sparse representation of player design matrices: see https://mc-stan.org/docs/2_29/functions-reference/CSR.html
  //Declaration
  vector[nzp] wpo;
  int vpo[nzp];
  int upo[ng+1];
...
model {
  ...
  y ~ bernoulli_logit(player_off+player_def);
