...
datalist <- list(ng=ng, y=y, np=np, XPO=XPO, XPD=XPD,
                 nzp=nzp, s=s, r=r)
...
pm_mod <- stan_model(file = here("model", "ppool_off-def-nt.stan"))
pm_fit <- sampling(object = pm_mod, 
                   data = datalist)

seasons <- pull_seasons(d_fname)
model_fname <- glue::glue("ppool_off-def-ng-nt_{seasons}_{lubridate::today()}.rds")
